...
async function fetchAndRenderBlogs() {
    try {
        ...
        blogsData.forEach((blog) => {
            ...
            const maxLength = 20;
            const shortDescription = blog.description.split(' ').slice(0, maxLength).join(' ');
            const truncatedDescription = blog.description.length > maxLength ? `${shortDescription}...` : shortDescription;
...
async function formSearch() {
    try {
        ...
        form_Search.addEventListener("submit", function (event) {
            ...
            if (!keyword && location === "Location" && category === "Category" && people === "People") {
                alert("Please provide all search criterion.");
                return;
            }

            var searchQuery = "?keyword=" + encodeURIComponent(keyword) +
